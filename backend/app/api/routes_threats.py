...
logger = get_logger("routes_threats")


@router.post("/analyze")
def analyze_threat(event: ThreatEvent):
    logger.info(
        f"Received threat analysis request for source_ip={event.source_ip}, event_type={event.event_type}"
    )
    ...
    logger.info(
        f"Threat analysis complete for source_ip={event.source_ip} with action={decision.action}"
    )
